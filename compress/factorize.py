...
def _to_low_rank_recursive(
    model: nn.Module, should_do: Callable, prefix=""
):
    modules_to_replace = []
    for name, module in model.named_children():
        ...
        if isinstance(module, nn.Linear):
            ...
            for idx, sub_module in enumerate(module):
                ...
                if isinstance(sub_module, nn.Linear) and should_do(sub_module, sub_full_name):
            ...
            modules_to_replace.extend(
                _to_low_rank_recursive(
                    module, should_do, prefix=full_name
                )
            )
...
def to_low_rank(
    ...
    modules_to_replace = _to_low_rank_recursive(
        model, should_do=should_do, prefix=""
    )
    if not inplace:
        model_initializer = kwargs.pop("model_initializer", None)
        assert model_initializer is not None, "model_initializer must be provided if inplace=False"
