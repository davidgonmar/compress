...
def main():
    ...
    print(
        f"Baseline → FLOPs: {flops_base}, Params: {n_params_base}, Metrics: {baseline}"
    )
    ...
    for x in (
        ...
        else ranks if args.metric == "rank" else perms
    ):
        ...
        print(
            f"{tag} → FLOPs: {flops_lr}, Params ratio: {n_params_lr/n_params_base:.4f}, Metrics: {res}"
        )

        # Save factorized model
        torch.save(
            model_lr.state_dict(), f"{args.model_name}_{args.task_name}_{tag}.pth"
        )
