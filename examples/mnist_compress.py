...
entropies = {name: singular_values_entropy(module.weight).item() for name, module in model.named_modules() if isinstance(module, torch.nn.Linear)}

print(f"Per-layer entropies: {entropies}")

singular_vals = {name: torch.linalg.svd(module.weight, full_matrices=False).S.tolist() for name, module in model.named_modules() if isinstance(module, torch.nn.Linear)}
...
for ratio in ratios:
    model_lr = to_low_rank(model, ratio_to_keep=ratio, inplace=False, model_initializer=lambda: SimpleMNISTModel().to(device))
    test_loss, test_acc = evaluate(model_lr, test_loader, criterion, device)
    weights = [module.weight for module in model_lr.modules() if isinstance(module, torch.nn.Linear)]
